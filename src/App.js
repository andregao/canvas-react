...
const useLocalStorage = (key, defaultValue) => {
  const previousValue = JSON.parse(localStorage.getItem(`canvas-app-${key}`));
  const [currentValue, setValue] = useState(previousValue || defaultValue);
  useEffect(() => {
    localStorage.setItem(`canvas-app-${key}`, JSON.stringify(currentValue));
  });
...
const useCanvas = () => {
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0, 0, window.innerHeight + 300, window.innerWidth + 300);
    locations.forEach(location => draw(ctx, location));
  });

  return [canvasRef, locations, setLocations];
};

function App() {
  const [ ref, locations, setLocations ] = useCanvas();
